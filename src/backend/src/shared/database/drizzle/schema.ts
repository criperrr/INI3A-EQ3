...
export const recoveryMessageStatus = pgEnum("recovery_message_status", ['Satisfatório', 'Insatisfatório', 'Não Compareceu', 'Não aconteceu'])
...
export const scope = pgTable("scope", {
	...
	unique("scope_scope_name_key").on(table.scopeName),
]);
...
export const user = pgTable("user", {
	...
	index("idx_user_location").using("gist", table.location.asc().nullsLast().op("gist_geography_ops")),
	index("idx_user_role_id").using("btree", table.roleId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const ocurrency = pgTable("ocurrency", {
	id: serial().primaryKey().notNull(),
	...
	index("idx_ocurrency_market_id_created_at_desc").using("btree", table.marketId.asc().nullsLast().op("timestamptz_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("idx_ocurrency_product_id_created_at_desc").using("btree", table.productId.asc().nullsLast().op("timestamptz_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	index("idx_ocurrency_unresolved_partial").using("btree", table.isResolved.asc().nullsLast().op("bool_ops"), table.createdAt.asc().nullsLast().op("timestamptz_ops")).where(sql`(is_resolved = false)`),
	foreignKey({
...
export const market = pgTable("market", {
	...
	index("idx_market_location").using("gist", table.location.asc().nullsLast().op("gist_geography_ops")),
]);
...
export const product = pgTable("product", {
	...
	index("idx_product_ean_partial").using("btree", table.ean.asc().nullsLast().op("text_ops")).where(sql`(ean IS NOT NULL)`),
	index("idx_product_name_trgm").using("gin", table.name.asc().nullsLast().op("gin_trgm_ops")),
	index("idx_product_ncm").using("btree", table.ncm.asc().nullsLast().op("text_ops")),
]);
...
export const roleScope = pgTable("role_scope", {
	roleId: integer("role_id").notNull(),
	...
	primaryKey({ columns: [table.scopeId, table.roleId], name: "role_scope_pkey"}),
]);
...
export const userBadge = pgTable("user_badge", {
	...
	primaryKey({ columns: [table.userId, table.badgeId], name: "user_badge_pkey"}),
]);
...
export const cartProduct = pgTable("cart_product", {
	...
	primaryKey({ columns: [table.productId, table.cartId], name: "cart_product_pkey"}),
]);

export const cured = pgTable("cured", {
	userId: integer("user_id").notNull(),
	...
	index("idx_cured_ocurrency_id_date_asc").using("btree", table.ocurrencyId.asc().nullsLast().op("int4_ops"), table.date.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
	...
	primaryKey({ columns: [table.userId, table.ocurrencyId], name: "cured_pkey"}),
]);
