...
## 📱 INI3A-EQ3 Project Mobile Context

When working on this workspace (`src/frontend`):
- **Framework & Routing:** React Native with Expo SDK and Expo Router (file-based routing in `app/`).
- **HTTP Layer:** Never call `fetch` directly. Always use `services/api.ts` (`apiRequest<T>`), which manages auto-refresh on 401 and tunnel bypass headers.
- **Design System:** Consume tokens from `.agents/DESIGN.md` via `useTheme()` (`themeStyles.bg`, `.card`, `.headerBg`, `.border`, `.text`, `.subText`, `.inputBg`, `accent`).
- **Core Features:** Camera barcode EAN scanning (`scannerProduct.tsx`), product price comparison (`ocurrency`), PostGIS proximity markets in Map (`map.native.tsx`).
- **Reference:** Read `.agents/skills/frontend/SKILL.md` and `.agents/DESIGN.md`.
