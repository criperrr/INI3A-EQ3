...
## 🗺️ INI3A-EQ3 Database & PostGIS Context

When working on this workspace (`src/backend/src/shared/database`):
- **Database:** PostgreSQL with PostGIS extension.
- **ORM:** Drizzle ORM (`shared/database/schema.ts`).
- **PostGIS Geography:** The custom Drizzle `geography` type accepts `{ lat, lng }` on write. On read, ALWAYS wrap with `sql`ST_AsGeoJson(${table.location})`` to avoid raw WKB binary data.
- **Spatial & Trigram Indexing:** GiST index on `market.location` for radius queries (`ST_DWithin`); GIN trigram index on `product.name` for fast fuzzy product search.
- **Entity Tables:** `role, scope, role_scope, user, badge, user_badge, product, market, ocurrency, cart, cart_product, cured`.
- **Reference:** Read `.agents/skills/postgresql/SKILL.md` and `.agents/skills/geo-fundamentals/SKILL.md`.
