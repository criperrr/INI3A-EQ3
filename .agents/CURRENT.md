...
- [ ] Sistema de Desvio Padrão Adaptativo para Preços e Retenção Antifraude com Moderação Admin (especificação técnica completa salva em `.agents/PLANO_SISTEMA_DESVIO_PADRAO_E_MONITORAMENTO.md`: janela temporal móvel de 45 dias, coeficiente de variação CV dinâmico, detecção de quórum multiusuário para acomodação automática de inflação sem falso-positivos, retenção de preços extremos para análise administrativa em `adminModeration.tsx`, e alertas interativos de confirmação em `registerProduct.tsx`).
